...
SELECT
    ...
    s.service_name,
    b.booking_date,
    b.service_city || ', ' || b.service_province AS service_location,
    COALESCE(
...
JOIN services s
    ON b.service_id = s.service_id
LEFT JOIN booking_technicians bt
    ON b.booking_id = bt.booking_id
LEFT JOIN technicians t
    ON bt.technician_id = t.technician_id;

/* Columns for Bookings Report
Booking Code
Customer Name
Service Name
Booking Date
Service Location (City, Province only)
Technician Name
Booking Status
*/
...
CREATE OR REPLACE FUNCTION total_completed_services()
RETURNS INT
LANGUAGE SQL
AS $$
    SELECT COUNT(*)
